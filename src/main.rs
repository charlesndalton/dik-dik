...
async fn main() -> Result<()> {
    let telegram_token =
        env::var("DIK_DIK_TELEGRAM_TOKEN").expect("DIK_DIK_TELEGRAM_TOKEN not set");
...
async fn daily_check(infura_api_key: &str, telegram_token: &str) -> Result<()> {
    loop {
        ...
        for individual_asset_report in report {
            let mut health_ratio = individual_asset_report.total_assets()
                / individual_asset_report.t_asset_total_supply();
            health_ratio.rescale(3);

            let message = format!(
                "{} health ratio: {}",
                individual_asset_report.asset_name(),
                health_ratio
            );
            println!("{}", message);
...
mod report_creator {
    use crate::{
        blockchain_client,
        types::{Error, IndividualAssetTokemakReport, Result, TokemakReport},
    };
    ...
    pub async fn create_report(infura_api_key: &str) -> Result<TokemakReport> {
        ...
        let coordinator_address =
            "0x90b6C61B102eA260131aB48377E143D6EB3A9d4B".parse::<Address>()?;
        let treasury_address = "0x8b4334d4812C530574Bd4F2763FcD22dE94A969B".parse::<Address>()?;

        let tokemak_strategies =
            eth_addresses["strategies"]["tokemak"]
                .as_object()
                .ok_or(Error::EthAddressesError {
                    expected_field: "strategies.tokemak".to_string(),
                })?;
        ...
        for (asset_name, strategy_address) in tokemak_strategies.iter() {
            let strategy_address = strategy_address
                .as_str()
                .ok_or(Error::EthAddressesError {
                    expected_field: format!("strategies.tokemak.{}.address", asset_name),
                })?
                .parse::<Address>()?;
            ...
            let t_asset_strategy_balance =
                blockchain_client::get_balance_of(&t_asset, strategy_address).await?;

            let mut accounts_to_check_for_free_assets =
                vec![t_asset.address(), manager_address, coordinator_address];
            ...
            let accounts_to_check_for_lp_tokens_with_impermanent_loss =
                vec![manager_address, coordinator_address];
            let assets_in_lp_tokens_with_impermanent_loss =
                blockchain_client::get_want_in_univ2_pools(
                    &want,
                    &eth_client,
                    &accounts_to_check_for_lp_tokens_with_impermanent_loss,
                )
                .await?;

            let accounts_to_check_for_lp_tokens_without_impermanent_loss =
                vec![manager_address, coordinator_address];
            let assets_in_lp_tokens_without_impermanent_loss =
                blockchain_client::get_want_in_curve_pools(
                    &want,
                    &eth_client,
                    &accounts_to_check_for_lp_tokens_without_impermanent_loss,
                )
                .await?;
            let total_assets = free_assets
                + assets_in_lp_tokens_with_impermanent_loss
                + assets_in_lp_tokens_without_impermanent_loss;

            report.push(IndividualAssetTokemakReport::new(
                asset_name.to_string(),
                t_asset_strategy_balance,
                t_asset_total_supply,
                total_assets,
                free_assets,
                assets_in_lp_tokens_without_impermanent_loss,
                assets_in_lp_tokens_with_impermanent_loss,
...
mod blockchain_client {
    use crate::types::{CurvePool as CurvePoolStruct, Result};
    use ethers::prelude::*;
    ...
    pub async fn get_want(client: &Client, strategy_address: Address) -> Result<IERC20> {
        let want_address = TokemakStrategy::new(strategy_address, Arc::clone(&client))
            .want()
            .call()
            .await?;
    ...
    pub async fn get_balance_of(token: &IERC20, address: Address) -> Result<Decimal> {
        ...
        let mut balance = Decimal::from_i128_with_scale(
            token
                .balance_of(address)
                .call()
                .await?
    ...
    pub async fn get_total_supply(token: &IERC20) -> Result<Decimal> {
        ...
        let mut total_supply = Decimal::from_i128_with_scale(
            token
                .total_supply()
                .call()
                .await?
    ...
    pub async fn get_want_in_univ2_pools(
        want: &IERC20,
        client: &Client,
        accounts_to_check: &Vec<Address>,
    ) -> Result<Decimal> {
        let univ2_pool_addresses = vec![
    ...
    pub async fn get_want_in_curve_pools(
        want: &IERC20,
        client: &Client,
        accounts_to_check: &Vec<Address>,
    ) -> Result<Decimal> {
        let curve_pools = vec![
            CurvePoolStruct::new(
                "0xd632f22692FaC7611d2AA1C0D552930D43CAEd3B".to_string(),
                "0xd632f22692FaC7611d2AA1C0D552930D43CAEd3B".to_string(),
                Some("0xB900EF131301B307dB5eFcbed9DBb50A3e209B2e".to_string()),
            ),
            CurvePoolStruct::new(
                "0x0437ac6109e8A366A1F4816edF312A36952DB856".to_string(),
                "0x0437ac6109e8A366A1F4816edF312A36952DB856".to_string(),
                None,
            ),
            CurvePoolStruct::new(
                "0x50B0D9171160d6EB8Aa39E090Da51E7e078E81c4".to_string(),
                "0x50B0D9171160d6EB8Aa39E090Da51E7e078E81c4".to_string(),
                None,
            ),
            CurvePoolStruct::new(
                ...
                "0xEd279fDD11cA84bEef15AF5D39BB4d4bEE23F0cA".to_string(),
                Some("0x2ad92A7aE036a038ff02B96c88de868ddf3f8190".to_string()),
            ),
        ];
        let mut want_owned_in_pools = Decimal::new(0, 6);
        ...
        for curve_pool in curve_pools {
            ...
            if lp_token_name.as_str().contains(want_symbol.as_str())
                || lp_token_symbol.as_str().contains(want_symbol.as_str())
            {
                ...
                for account in accounts_to_check {
                    ...
                    if curve_pool.convex_rewards_pool_address().is_some() {
                        let convex_rewards_pool_address = curve_pool
                            .convex_rewards_pool_address()
                            .as_ref()
                            .unwrap()
                            .parse::<Address>()?;
                        let rewards = IERC20::new(convex_rewards_pool_address, Arc::clone(&client));
                        owned_lp_tokens += Decimal::from_i128_with_scale(
                            rewards
                                .balance_of(*account)
                                .call()
                                .await?
                ...
                let virtual_price = Decimal::from_i128_with_scale(
    ...
    async fn get_t_asset_address(client: &Client, strategy_address: Address) -> Result<Address> {
        let weth_strategy_address =
            "0x2EFB43C8C9AFe71d98B3093C3FD4dEB7Ce543C6D".parse::<Address>()?;

        if strategy_address == weth_strategy_address {
            Ok("0xD3D13a578a53685B4ac36A1Bab31912D2B2A2F36".parse::<Address>()?)
        } else {
            Ok(TokemakStrategy::new(strategy_address, Arc::clone(&client))
                .t_asset()
                .call()
                .await?)
        }
...
mod types {
    ...
    pub enum Error {
        ...
        EthAddressesError { expected_field: String },
    }
